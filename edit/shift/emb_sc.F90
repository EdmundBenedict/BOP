...
!       REMOVE THIS
      ...
      real(dp) :: beta,wc,rms,ecorr,sumzb,sumzo,sumzde
...
!            sumzde = 0.0_dp
            ...
            do ia = 1,mbc%n(1)
                 de(ia) = de(ia) - sumzde/(sumzb+sumzo)
             enddo



             do ia = 1,mbc%n(2)
                 if (mbc%orth(ia) == 1) de(ia+mbc%n(1)) = de(ia+mbc%n(1)) -sumzde/(sumzo+sumzb)
             enddo
...
!  When ovl==true the tb part requires more neighbours due to greater range of s-orb
      ...
      do isp = 1, nsp         
          ...
          call system_clock(c2)
          if (.not. quiet) print *, 't(intcycle - mbf):', real(c2-c1,dp)/real(cr,dp)   
      ...
      if (.not. quiet) print *, 't(allreduce - mbf):', real(c4-c3,dp)/real(cr,dp)
...
!  Calculate TB GF from expansion coefficients in Dyson case          
          if (.not. mbc% mxgf) then
              ...
              call system_clock(c2)
              if (.not. quiet) print *, 't(diagham - mbenergy):', real(c2-c1,dp)/real(cr,dp)
!               if (.not. quiet) then
!                 print *, 'eigvals',isp
!                 print *, mbc%en(:,isp)
!               endif
...
!   Calculate onsite energy and charges for promotion energy
      ...
      call system_clock(c4)
      if (.not. quiet) print *, 't(intcycle - mbenergy):', real(c4-c3,dp)/real(cr,dp)
...
!       call mbosprom(mbc,efermi,efermb,bsons,eprom)
      ...
      buf = [ebond, eprom, bsons,eprom0,epromb,epromo]
      call mpi_allreduce(mpi_in_place, buf, 6, mpi_real8, mpi_sum, mpi_comm_world, ierr)
     ...
     epromo =  buf(6)
      ...
      if (.not. quiet) print *, 't(allreduce - mbenergy):', real(c6-c5,dp)/real(cr,dp)
...
!      endif
   ...
   subroutine mbprom(mbc,q,ql,isp,mbeprom,mbeprom0,mbepromb,mbepromo) 
...
!       use mod_ham
      ...
      real(dp), intent(inout) :: mbeprom0,mbepromb,mbepromo
      real(dp) :: ela0
...
!               print *,iproc, 'nonorth',ql(:,Tia)
          ...
          do la = 1,nlt
              ...
              if (mbc%orth(tia) == 1) mbepromo = mbepromo + ela
          enddo             
      enddo
...
!       call mbq
   ...
   function embeff(mbc,locc,ef0) result(ef)
      use mod_all_scalar, only : totnia,nsp,mfac,quiet,kt
      ...
      integer :: info,liwork,lwork,isp,ierr
      integer, allocatable :: iwork(:)
...
!             zp   = zfac*zp
         ...
         call system_clock(c2)
         if (.not. quiet) print *, 't(intcycle - mbq):', real(c2-c1,dp)/real(cr,dp)
...
!    potential ISSUE: may need to reduce rho before i calc. q
      ...
      call system_clock(c4)
      if (.not. quiet) print *, 't(allreduce - mbq):', real(c4-c3,dp)/real(cr,dp)
...
!       print *, iproc,'after dqchi',mbc%dqchi(:mbc%nd)
      call system_clock(c6)
      if (.not. quiet) print *, 't(mbq):', real(c6-c5,dp)/real(cr,dp)
