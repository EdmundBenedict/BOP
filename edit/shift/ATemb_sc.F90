...
!                         posham,   aptr_tb   bptr_tb   aptr_dh  bptr_dh 
      ...
      complex(dp), allocatable :: a(:,:), ac(:,:), gt(:,:),at(:,:)!,hp(:,:)!In short:
!      a = G_BOP (-dh')
!      gt = (z - (h - dh a))^{-1}
!      ac = a gt
...
!       calc the diagonal here; do over atoms and orbs  gb(ia,ia) +-  sum_ib(ac(ia,ib) * a(ia,ib))
          do i = 1, mbc % bhsz  
              do j = 1, mbc % thsz
                  mbc % ggd(i) = mbc % ggd(i) + mbc % ac(i,j)*(mbc % at(j,i))
              end do
          end do
      endif
                ...
                if (map(ib) == ib .and. mbc%mxgf) then
                    do i = 1, mbc % pbh(ia+1) - mbc % pbh(ia)
                        do j = 1, mbc % pbh(ib+1) - mbc % pbh(ib)
                            gab(i,j) = gab(i,j) + sum(mbc % ac(mbc % pbh(ia) + i, :) * (mbc % at(:,mbc % pbh(ib) + j)))
                        end do
                    end do
                endif 
                      ...
                      if (jb /= 1) then !jb == 1 => ib == ia THIS HAS BEEN CHANGED TO JB /=1
                          ...
                          if (map(ib) == ib .and. mbc%mxgf) then
                              do i = 1, mbc % pbh(ia+1) - mbc % pbh(ia)
                                  do j = 1, mbc % pbh(ib+1) - mbc % pbh(ib)
                                      gab(i,j) = gab(i,j) + &
                                      &    sum(mbc % ac(mbc % pbh(ia) + i, :) * (mbc % at(:,mbc % pbh(ib) + j)))
...
! CARRIED OUT IN MBQ NOW
   ...
   subroutine gb_x_dh(mbc,ez,isp)
      ...
      integer :: ib, ia, it, jb, jt, la, lb, lt, gia, gib, git,i,j,jb2

      complex(dp) :: gab(mxnstat, mxnstat),gab2(mxnstat, mxnstat)
...
!          !          print *, "jbs",ia,ib,jb
                  ...
                  if (ia /= ib) then
                    ...
                    call assoc_ham(ib)
                    jb2 = decipher(ia)
                    call get_gab(ez, ib, ia, jb2, gab2,.false.)
                    call assoc_ab(ia,isp)
                    call assoc_ham(ia)
           !          
           !          
...
!                               print *, ia,git,ib,(gab(la - mbc % pbh(ia), lb) ),mbc%dh(lt - mbc % pth(it),lb,jt)
                          ...
                          mbc%at(lt,la) = mbc%at(lt,la) - (gab2(lb,la - mbc % pbh(ia)) )* mbc%dh(lt - mbc % pth(it),lb,jt)
                        end do
                     end do
                  end do
...
!        a(:,:), ac(:,:), aca(:,:), v(:), w(:,:), occ(:)
      ...
      if (mbc%mxgf) then
        ...
        if (.not. allocated(mbc%a)) allocate(mbc % at(mbc % thsz, mbc % bhsz))
