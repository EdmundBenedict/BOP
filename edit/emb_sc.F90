...
!               mbc % dqchi(ia) = mbc % qmpol(1,ia-mbc%rlst(2)+1)/mbc % dqchi(ia)
              if (.not. quiet) then
                    ...
                    print *,'orth',mbc % orth(ia - mbc%n(1))
                    print *, 'dq,qmpol,q',dq(ia) , mbc %qmpol(1,ia-mbc%rlst(2)+1) , (mbc % qmpol(1,ia-mbc%rlst(2)+1)+zct)
                    print *, zct
                    print *, ' '
                    if (abs(dq(ia)) > 6) then
...
!       m     = nint(mfac*0.25_dp*1.8_dp/kt)
      ...
      if (.not. allocated(si) .and. mbc%ovl) allocate(si(mbc%thsz,mbc%thsz))
...
!           write(6,'("STARTING COORD:")')
!           do ia = 1, mbc%nd
!               write(6,'(3(x,f20.16),x,f14.12)') ad(:, ia),mg(ia) 
!           enddo
! 
!       endif
