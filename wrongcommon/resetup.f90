...
!      subroutine emb_reloadtb( ham_conf )
!          
!       use mod_precision
!       use mod_all_scalar
!       use mod_const
...
!          zin = ham_conf % a(j) % z
!          
!          tbzc(zin) = ham_conf % a(j) % b % cc 
!          tbnl(zin) = ham_conf % a(j) % b % norbs
!          tbllist(:,zin) = ham_conf % a(j) % b % orblist
!          tbnstt(zin) = 0
!          
!          if (mag) tbistn(zin) = ham_conf % a(j) % b % st
!          
!          do i = 1,tbnl(zin)
!             tbnstt(zin) = tbnstt(zin) + 2*tbllist(i,zin)+1
!          enddo
! 
!          tbes(zin) = ham_conf % a(j) % b % es
!          tbep(zin) = ham_conf % a(j) % b % ep
!          tbed(zin) = ham_conf % a(j) % b % ed
!       end do
! 
!       
!    !       
...
!             subroutine emb_reloadtb(tbham_conf)
!                   import ham_conf_t
...
!                
   ...
   ! bsort(z_ord_btype) -> conf_bond_idx
         asort = -1
         do i = 0, run_conf % ham_conf % na - 1
            do j = 0,natype
                  if (atype(j) == run_conf % ham_conf % a(i) % z) then
                     asort(j) = i
                     exit
                  endif
            end do
         ...
         do i = 0, run_conf % ham_conf % nb - 1
            bsort(btype(run_conf%ham_conf%b(i)%atom0%z, run_conf%ham_conf%b(i)%atom1%z)) = i
         enddo
...
!          if (potflg == 9) then
!              call emb_reloadtb(run_conf % tbham_conf )
!          endif
