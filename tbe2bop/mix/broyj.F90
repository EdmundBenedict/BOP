...
! C     double precision wl(nn,3),u(nn,nn),v(nn,nn)
      ...
      if (ir .gt. nn) then
          print *, 'broyj: increase nn, need',ir
          call panic()
...
! C#ifdef AWRITE
            ...
            print *, ' broyj:  max shift = ',diff
            print *, '  is larger than dxmx = ',dxmx
            print *, 'Scale by ', dxmx/diff
!             call awrit3(' broyj:  max shift = %1;3g'//
!      .        ' is larger than dxmx = %1;3g.  Scale by %1;3g',
...
!      .    i1mach(2),xmax,gmax,ir)
          ...
          print *, ' broyj: converged to max dx ',xmax
          print *, 'gmax=',gmax,' using ',ir,' iterations'
      elseif (ir .ne. 1) then
!         call awrit4(' broyj:  ir=%i  dxmax=%1;2g  gmax=%1;2g'//
!      .    '  wc=%1;2g',' ',80,i1mach(2),ir,xmax,gmax,wc)
          print *, ' broyj:  ir=',ir,'  dxmax=',xmax,' gmax=',gmax,' wc=',wc
