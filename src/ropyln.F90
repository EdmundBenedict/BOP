...
! C ----------------------------------------------------------------------
      implicit none
      integer nd,n,m,lmax,l,kk
! c     integer i,ocm,osm,oq,oh
      double precision x(n),y(n),z(n),yl(nd,(lmax+1)**2),rsq(n),cx(3)
      double precision fpi,f2m
      double precision, allocatable:: cm(:),sm(:),q(:),h(:)

! C     call tcn('ropyln')
      if (n .gt. nd) stop 'ropyln: nd too small'
      fpi = 16*datan(1d0)
...
!           print *, 'm,l,',m,l
          ...
          cx(2) = cx(1)
          cx(1) = cx(1)* &
     &            dsqrt(dble((l+1-m)*(2*l+3))/dble((l+1+m)*(2*l+1)))
        enddo
      enddo
! c     call rlse(ocm)
