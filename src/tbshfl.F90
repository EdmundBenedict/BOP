...
! c... Checks
      if (n .gt. nm) stop ' tbshfl: incompatible dimensions'
...
!      .    ,job) 
         stop ' tbshfl: parameter job should be either 0 or 1.'
      endif
...
! c... Checks
      if (max(n,m) .gt. (lmax+1)**2) stop ' xsqfac: increase parameter lmax'
      if (n .gt. nm) stop ' strfac: incompatible dimensions'
!       call isanrg(job,0,1,' strfac:','job',.true.)
      if ( job < 0 .or. job > 1) then
          stop 'strfac: unexpected value for job'
      endif
