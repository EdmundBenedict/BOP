...
!
      ...
      integer ia,ib,ja,ja0,la,lb
      integer za,zb,mapb,llista(ml),llistb(ml)
...
!
!    Evaluate the band structure energy, forces and bond orders.
!
!    For each bond do ..
!
...
!
            ...
            dea = de(ia)
            call matel(za,nla,nstta,llista,zb,nlb,nsttb,llistb,dr,nstta,nsttb,ksubh,dea, & 
     &                 mxnstat,ml,llista,llistb)

!      This is wrong argument list
            call grdmat(kgrad,dr,za,nla,nstta,llista,zb,nlb,nsttb,llistb,mxnstat,ml,llista,llistb)
...
!
     ...
     &                     + kk(3,ik)*dr(3)
                     ...
                     do while((in <= kmxh).and.(occ(in,ik) > 1.0e-6_dp))
                        ...
                        rcc = (real(kpsi(iha,in,ik))* & 
     &                         real(kpsi(ihb,in,ik))+ & 
     &                         aimag(kpsi(iha,in,ik))* & 
     &                         aimag(kpsi(ihb,in,ik)))*fac
                        icc = (real(kpsi(iha,in,ik))* & 
