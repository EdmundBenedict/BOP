...
!        individual atomic forces for the gamma-surface relaxation.         *
!        May also be used for the grain boundary relaxation.                *
!                                                                           *
!                                                                           *
!****************************************************************************


      subroutine maxf_gs( maxf )
          use mod_precision
...
!             Average forces acting on the atoms in layer K
              ...
              do  j = 1, 3
                  flayer( j ) = ( fblock( j, finlay( k ) ) - &
     &               fblock( j, finlay(k)+ninlay(k) )) / ninlay(k)
              enddo
...
!  If we are calculating the gamma-surface, we shouldn't allow
!  layers move as a whole parallel to the cut.
...
!  Compare those two maximum forces.
      ...
      if ( fmax1  >  fmax2 )  then
          maxf = fmax1
          if ( pointsgs  ==  1  .or.  mvflag  ==  15 ) then
