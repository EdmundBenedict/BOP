...
! C ----------------------------------------------------------------
      ...
      character*1 cs
      integer nmin,m1,m2,i,j,isw,nmin0,wipv(n)
      save nmin
! C#ifdefC DEBUG
! C      character*10 fmt
! C      data fmt /'(8f16.10)'/
...
!           print *, w(:,1)
          ...
          if (n .le. 2) then
            ...
            call dgedi(a,lda,n,wipv,xx,xx,1)
          else
!             print *, w(1,2)
            call dgedi(a,lda,n,wipv,xx,w(1,2),1)
          endif
        else
          call dsifa(a,lda,n,wipv,ierr)
          if (ierr .ne. 0) return
          call dsidi(a,lda,n,wipv,xx,i,w(1,2),1)
          do  10  i = 1, n
          do  10  j = 1, i
   10     a(i,j) = a(j,i)
        endif
...
! C --- R1 = a11^-1 in a11 ---
      if (m1 .lt. nmin .or. nlev .eq. 0) then
        if (cs .eq. 's') then
          call dsifa(a,lda,m1,wipv,ierr)
          if (ierr .ne. 0) return
          call dsidi(a,lda,m1,wipv,xx,i,w(1,2),1)
          do  12  i = 1, n
          do  12  j = 1, i
   12     a(i,j) = a(j,i)
        else
          ...
          wipv =  w(:,1)
          if (ierr .ne. 0) return
          call dgedi(a,lda,m1,wipv,xx,w(1,2),1)
        endif
      else
! C#ifdefC DEBUG
