...
!            do ia = 1,mbc%nd
!                if (ia <= mbc%n(1) .or. mbc%orth(ia-mbc%n(1)) == 1) &
!                          & de(ia) = de(ia) - sumzde/sumzo
!            enddo
...
!     do j = 1, mbc % thsz
!         do i = 1, mbc % bhsz
!            mbc % ac(i,j) = sum(mbc % a(i,:) * mbc % gt(:,j))
!         end do
!      end do
...
!               if (abs(dq(ia)) > abs(dqmaxb)) dqmaxb = dq(ia)
              ...
              if (.not. quiet) print *, 'bop',ia, dq(ia),de(ia)
          enddo 
...
!               mbc % dqchi(ia) = mbc % qmpol(1,ia-mbc%rlst(2)+1)/mbc % dqchi(ia)
              if (.not. quiet) then
                    ...
                    print *, 'TB dq:',ia,dq(ia),mbc % orth(ia - mbc%n(1)),de(ia)
              endif
