...
!             
!:             if (.not. quiet) print *, 'sumzde bop:',sumzde,sumzo,sumzde/sumzo
!             do ia = 1,mbc%n(2)
...
!       end function mbeprom
      ...
      function mbent(mbc,ef,efb) result(entropy)
        ...
        use mod_ham
        use mod_clock
        use topologia, only : iproc,nproc, master, aproc, sdistrib
...
!          zfac  = zp*zp
      ...
      call system_clock(c1)
!      allocate(szs(0:aproc-1)
      allocate(szs(0:nproc-1))
      displs = displs * mbc % nd

!     szs = displs(1:aproc) - displs(0:aproc-1)
...
!               if (abs(dq(ia)) > abs(dqmaxb)) dqmaxb = dq(ia)
              if (mbc%mxgf) mbc % dqchi(ia) = dq(ia)/mbc % dqchi(ia)
               if (.not. quiet) print *, 'BOP dq:',ia, dq(ia)
          enddo 
...
!                  endif
 ...
 !             endif
              if (.not. quiet) print *, 'TB dq:',ia,dq(ia),'orth:',mbc%orth(ia - mbc%n(1))   
...
!       call mbq
   ...
   function embeff(mbc,locc,ef0) result(ef)
      ...
      integer :: info,liwork,lwork,isp!,ncall
      integer, allocatable :: iwork(:)
...
!         if (mbc % sctb .and. diff < 1d6*maxdn) chi = .true.
      ...
      diff = abs(nel-locc)
!      ncall = ncall + 1
!      if (diff > maxdn .and. ncall < 2) goto 2
...
!               if ( ia /= 174 .and.  ia /= 175 .and. ia /= 178 .and. ia /= 179 .and. &
!                    ia /= 190 .and.  ia /= 191 .and. ia /= 194 .and. ia /= 195 ) &
               & mbc%orth(ia) = 1
!              if(.not. quiet) then
