...
!       REMOVE THIS
      ...
      real(dp) :: dqmax_old, aa, aamin, aamax,det,deb,dea,sumzo,sumzde
...
!         if (.not. quiet) then
!           print *, 'NEW BOP NEBLIST'
!           call print_neblist(1, mbc % n(1), mbc % apb, mbc % bpb)
!         endif
        if (.not. quiet) print *, 'NOT CHANGING BOP NLIST'
     endif
...
!          TB->BOP intersites
    ...
    !             print *, "delta",git,(jt-jt0)+jtl,ib
                ...
                if (mbc%mxgf) then 
                    ...
                    jb = decipher(ib)
                    call get_gab(ezb, git, ib, jb, gab,.false.)
                endif
...
!                   print *, mbc % rho(i,j,(ja-ja0),gia), grad(1:3,i,j)
           ...
           ! IF (.NOT. QUIET) print *, "BOP,ia,ib", ia,ib
!             print *, 'dr',dr
!             print *, fab
!             print *, ' '
...
!           REMOVED SO BOP WOULD CALC HALF OF FORCE
!                 if (gib <= mbc%n(1)) then
!                     fbs(:,git) = fbs(:,git) + 2*fab
!                     fbs(:,gib) = fbs(:,gib) - 2*fab
!                 else 
                if (map(gib) == 0) then
                    fbs(:,git) = fbs(:,git) + 2*fab
                else if (.not. mbc%mxgf) then
                    fbs(:,git) = fbs(:,git) + fab
                    fbs(:,map(gib)) = fbs(:,map(gib)) - fab
                else
                    fbs(:,git) = fbs(:,git) + 2*fab
                    fbs(:,gib) = fbs(:,gib) - 2*fab
                endif
            ...
            !             if (mbc%mxgf .or. .not. mbc%allbop) then
                  ...
                  do while (ib /= eol)
                      ...
                      if (mbc%mxgf) then
                          ...
                          jb = decipher(ib)
                          call get_gab(zepb, git, ib, jb, gab,.false.)
                          
                          nsttb = nstt(z(ib))
                      endif
!                 & - conjg(transpose(mbc % ac(mbc % pbh(ib) + 1 : mbc % pbh(ib + 1), mbc % pth(it) + 1 : mbc % pth(it + 1))))

!                 TAKEN OUT SO BOP WOULD CALC HALF OF THE ENERGY
!                       if (map(ib) == ib) gab = gab * 2
                      if (mbc%mxgf) gab = gab * 2
...
!   Calculate onsite energy and charges for promotion energy
          ...
          call mbprom(mbc,q,ql,isp,eprom,eprom0,epromb,epromo)

!           call mpi_allreduce(mpi_in_place,  q, mbc%n(1), mpi_real8,mpi_sum,mpi_comm_world, ierr) 
!           if (.not. quiet) then
!             print *, 'EPROM CHARGES'
!             do ia = 1, mbc%n(1)
!                 print *,'old',mbc%q(isp,ia),'new',q(ia)
!             enddo
!           endif
...
!       call mbosprom(mbc,efermi,efermb,bsons,eprom)
      ...
      if (nproc > 1) then
           call mpi_allreduce(mpi_in_place,  epromb, 1, mpi_real8,mpi_sum, mpi_comm_world, ierr)
           call mpi_allreduce(mpi_in_place,  epromo, 1, mpi_real8,mpi_sum,mpi_comm_world, ierr)
...
!             buf(1,1) = ebond
   ...
   subroutine mbprom(mbc,q,ql,isp,mbeprom,mbeprom0,mbpromb,mbpromo) 
...
!       use mod_ham
      ...
      real(dp), intent(inout) :: mbeprom0,mbpromb,mbpromo
      real(dp) :: ela0
...
!               print *,iproc, 'nonorth',ql(:,Tia)
          ...
          do la = 1,nlt
              ...
              if (mbc%orth(tia) == 1) mbpromo = mbpromo + ela
...
!       end function mbeprom
      ...
      function mbent(mbc,ef,efb) result(entropy)
        use mod_all_scalar, only : kt, mfac, nsp,nrec
        use mod_atom_ar
        use mod_precision          
        use mod_const
        use mod_srt
...
!          end function zentden
      ...
      if (rtc % rcut(1) /= rtc % rcut(2)) then
          ...
          mbc % apb = 0
          mbc % bpb = 0
          call extract_boplist(aptr, bptr,mbc % rlst, mbc % apb, mbc % bpb,mbc%nd,.true.)
      endif
...
!             pwp => hamtb % b(bt) % pwp
            ...
            epair = epair + fv(0)
            if (gib == ib .and. ib <= mbc%n(1)) epair = epair + fv(0)
...
!                 call tailed_fun(r, pwp % fp, pwp % tl, df, fv)
                ...
                if (gib == ib .and. ib <= mbc%n(1)) fpp(:,ib) = fpp(:,ib) - fv(1)*dr/r
            endif
...
!         enddo
   ...
   subroutine gtbi(mbc, ez, isp)
      use mod_all_scalar, only : mag,quiet
...
!       end do
      ...
      do it1 = 1, mbc % n(2)
         pt1 = mbc % pth(it1)
         nstt1 = mbc % pth(it1+1) - pt1
!          if (.not. quiet) print *, 'it1:',it1,'orth:',mbc%orth(it1)
         do it2 = 1, mbc % n(2)
            pt2 = mbc % pth(it2)
            nstt2 = mbc % pth(it2+1) - pt2
!             if (.not. quiet) print *, '   it2:',it2
...
!              endif
              if (.not. quiet) print *, 'BOP dq:',ia,  dq(ia),de(ia)
          enddo 
...
!               endif
              ...
              if (.not. quiet) print *, 'TB DQ:',ia,dq(ia),mbc%orth(ia-mbc%n(1)),de(ia)
...
!       endif
   ...
   function mbq(mbc, ef) result(cnel)
            ...
            if (.not. mbc%allbop) then
                ...
                do ia = 1, mbc % nd
                  do la = mbc%pah(ia)+1, mbc%pah(ia+1)
                      ...
                      if (mbc%sctb) mbc % dqchi(ia) = mbc%dqchi(ia)+real(zpn*mbc%ggd(la)*mbc%ggd(la))
                  end do
                end do
            else
                do ia = 1, mbc % nd
                    if (zc(z(ia)) /= 0) then
                        ...
                        do la = 1,nstta      
                            ...
                            if (mbc%sctb) mbc%dqchi(ia) = mbc%dqchi(ia) + real(zpn * gab(la,la) * gab(la,la))
...
!       print *, 'thsz:', mbc%thsz
      ...
      if (.not. allocated(mbc% h)) then
          allocate(mbc%h(mbc%thsz,mbc%thsz,nsp))
          print *,' ALLOCATING H'
      endif
